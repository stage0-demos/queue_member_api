...
## run api server in dev mode - captures command line, serves API at localhost:8292
pipenv run dev

## run E2E tests (assumes running API at localhost:8292)
pipenv run e2e
...
```bash
# Get a token
export TOKEN=$(curl -s -X POST http://localhost:8292/dev-login \
  -H "Content-Type: application/json" \
  -d '{"subject": "user-123", "roles": ["admin"]}' | jq -r '.access_token')

# Get the API Configuration
curl http://localhost:8292/api/config \
