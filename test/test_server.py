...
class TestServerInitialization(unittest.TestCase):
    ...
    def test_config_singleton_initialized(self, mock_get_config, mock_get_mongo, mock_signal):
        ...
        mock_config.MEMBER_API_PORT = 8184
...
class TestAppConfiguration(unittest.TestCase):
    ...
    def test_curriculum_routes_registered(self):
        """Test that /api/curriculum routes are registered."""
    ...
    def test_review_routes_registered(self):
        """Test that /api/review routes are registered."""
    ...
    def test_all_blueprints_registered(self):
        ...
        self.assertIn('curriculum_routes', blueprint_names)

        self.assertIn('rating_routes', blueprint_names)

        self.assertIn('review_routes', blueprint_names)
    ...
    def test_url_map_contains_expected_routes(self):
        ...
        self.assertTrue(any('/api/curriculum' in rule for rule in rules))

        self.assertTrue(any('/api/rating' in rule for rule in rules))

        self.assertTrue(any('/api/review' in rule for rule in rules))
...
class TestServerExecution(unittest.TestCase):
    ...
    def test_main_execution_uses_config_port(self, mock_config, mock_run):
        """Test that __main__ execution uses MEMBER_API_PORT from config."""
        # Arrange
        mock_config.MEMBER_API_PORT = 9999
        ...
        if hasattr(server_module, '__name__'):
            ...
            api_port = mock_config.MEMBER_API_PORT
