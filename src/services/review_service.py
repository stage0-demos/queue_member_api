"""
Review service for business logic and RBAC.

Handles RBAC checks and MongoDB operations for Review domain.
"""
...
class ReviewService:
        ...
        Example RBAC implementation:
            if operation == 'update':
                ...
                if 'admin' not in token.get('roles', []):
                    raise HTTPForbidden("Admin role required to update review documents")
            elif operation == 'create':
                # Create requires staff or admin role
                if not any(role in token.get('roles', []) for role in ['staff', 'admin']):
                    raise HTTPForbidden("Staff or admin role required to create review documents")
            elif operation == 'read':
    ...
    @staticmethod
    def create_review(data, token, breadcrumb):
        """
        ...
        Returns:
            str: The ID of the created review document
        """
        try:
            ReviewService._check_permission(token, 'create')
            ...
            review_id = mongo.create_document(config.REVIEW_COLLECTION_NAME, data)
            logger.info(f"Created review { review_id} for user {token.get('user_id')}")
            return review_id
            ...
            raise HTTPInternalServerError(f"Failed to create review: {error_msg}")
    
    @staticmethod
    def get_reviews(token, breadcrumb, name=None, after_id=None, limit=10, sort_by='name', order='asc'):
        """
        Get infinite scroll batch of sorted, filtered review documents.
        ...
        """
        try:
            ReviewService._check_permission(token, 'read')
            mongo = MongoIO.get_instance()
            config = Config.get_instance()
            collection = mongo.get_collection(config.REVIEW_COLLECTION_NAME)
            ...
            logger.info(
                f"Retrieved {len(result['items'])} reviews (has_more={result['has_more']}) "
            ...
            raise HTTPInternalServerError("Failed to retrieve reviews")
    
    @staticmethod
    def get_review(review_id, token, breadcrumb):
        """
        Retrieve a specific review document by ID.
        ...
        Raises:
            HTTPNotFound: If review is not found
        """
        try:
            ReviewService._check_permission(token, 'read')
            ...
            mongo = MongoIO.get_instance()
            config = Config.get_instance()
            review = mongo.get_document(config.REVIEW_COLLECTION_NAME, review_id)
            if review is None:
                raise HTTPNotFound(f"Review { review_id} not found")
            
            logger.info(f"Retrieved review { review_id} for user {token.get('user_id')}")
            return review
        except HTTPNotFound:
            raise
        except Exception as e:
            logger.error(f"Error retrieving review { review_id}: {str(e)}")
            raise HTTPInternalServerError(f"Failed to retrieve review { review_id}")
    
    @staticmethod
    def update_review(review_id, data, token, breadcrumb):
        """
        ...
        Returns:
            dict: The updated review document
            
        Raises:
            HTTPNotFound: If review is not found
        """
        try:
            ReviewService._check_permission(token, 'update')
            ReviewService._validate_update_data(data)
            ...
            logger.info(f"Updated review { review_id} for user {token.get('user_id')}")
            return updated
        except (HTTPForbidden, HTTPNotFound):
            raise
        except Exception as e:
            logger.error(f"Error updating review { review_id}: {str(e)}")
            raise HTTPInternalServerError(f"Failed to update review { review_id}")