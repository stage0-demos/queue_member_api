...
class RatingService:
    ...
    - Business logic for Rating domain
    """
        ...
        Example RBAC implementation:
            if operation == 'update':
                ...
                if 'admin' not in token.get('roles', []):
                    raise HTTPForbidden("Admin role required to update rating documents")
            elif operation == 'create':
                # Create requires staff or admin role
                if not any(role in token.get('roles', []) for role in ['staff', 'admin']):
                    raise HTTPForbidden("Staff or admin role required to create rating documents")
            elif operation == 'read':
    ...
    @staticmethod
    def create_rating(data, token, breadcrumb):
        """
        ...
        Returns:
            str: The ID of the created rating document
        """
        try:
            RatingService._check_permission(token, 'create')
            ...
            rating_id = mongo.create_document(config.RATING_COLLECTION_NAME, data)
            logger.info(f"Created rating { rating_id} for user {token.get('user_id')}")
            return rating_id
        except HTTPForbidden:
            raise
        except Exception as e:
            error_msg = str(e)
            logger.error(f"Error creating rating: {error_msg}")
            raise HTTPInternalServerError(f"Failed to create rating: {error_msg}")
    
    @staticmethod
    def get_ratings(token, breadcrumb, name=None, after_id=None, limit=10, sort_by='name', order='asc'):
        """
    ...
    @staticmethod
    def update_rating(rating_id, data, token, breadcrumb):
        """
        Update a rating document.
        ...
        Returns:
            dict: The updated rating document
            
        Raises:
            HTTPNotFound: If rating is not found
        """
        try:
            RatingService._check_permission(token, 'update')
            RatingService._validate_update_data(data)
            ...
            logger.info(f"Updated rating { rating_id} for user {token.get('user_id')}")
            return updated
        except (HTTPForbidden, HTTPNotFound):
            raise
        except Exception as e:
            logger.error(f"Error updating rating { rating_id}: {str(e)}")
            raise HTTPInternalServerError(f"Failed to update rating { rating_id}")